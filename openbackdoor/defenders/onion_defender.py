...
class ONIONDefender(Defender):
    ...
    def correct(
        ...
        # self.threshold = self.compute_threshold(clean_dev)
    ...
    def detect(
        ...
        self.threshold = self.compute_threshold(clean_dev)
        logger.info("Constrain FRR to {}, threshold = {}".format(self.frr, self.threshold))
        ...
        for (poison_text, label, poison_label) in poison_data:
            ...
            if not np.isnan(score) and score:
                poison_score.append(score)
                poison_labels.append(poison_label)
        auroc = calculate_auroc(poison_score,poison_labels)
        logger.info("auroc: {}".format(auroc))
        plot_score_distribution(poison_score,poison_labels,self.poison_dataset+'-'+self.attacker)

        preds = np.zeros(len(poison_data))
        poisoned_idx = np.where(poison_score > self.threshold)
    ...
    def compute_score(self,orig_text):
        ...
        def get_PPL(text):
            ...
            ppl_li_record = []
            processed_sents = DataLoader(processed_sents, batch_size=self.batch_size,
    ...
    def compute_threshold(self,clean_dev):
        sents = [d[0] for d in clean_dev]
        scores = [self.compute_score(s) for s in sents]
        threshold_idx = int(len(sents) * (1-self.frr))
