...
class RAPDefender(Defender):
    ...
    def detect(
        ...
        self.construct(clean_dev[: half_dev])
        clean_prob = self.rap_prob(self.model, clean_dev[half_dev:])
        poison_prob = self.rap_prob(self.model, poison_data, clean=False)

        poison_labels = [d[2] for d in poison_data]
        auroc = calculate_auroc(poison_prob,poison_labels)
        logger.info("auroc: {}".format(auroc))
        plot_score_distribution(poison_prob,poison_labels,self.poison_dataset+'-'+self.attacker)
