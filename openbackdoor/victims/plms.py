...
class PLMVictim(Victim):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.model_config = AutoConfig.from_pretrained('./PTM/' + path)
        self.model_config.num_labels = num_classes
        # you can change huggingface model_config here
        self.plm = AutoModelForSequenceClassification.from_pretrained('./PTM/' + path, config=self.model_config)
        self.max_len = max_len
        self.tokenizer = AutoTokenizer.from_pretrained('./PTM/' + path)
        self.to(self.device)
