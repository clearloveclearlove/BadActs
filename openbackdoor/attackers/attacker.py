    ...
    def eval(self, victim: Victim, dataset: List, defender: Optional[Defender] = None):
        ...
        if defender is not None and defender.pre is False:
            ...
            if defender.correction:
                ...
                # post tune defense
                detect_poison_dataset = self.poison(victim, dataset, "detect")['test-detect']

                # filter
                # total_nums = len(detect_poison_dataset)
                # protected_poison_dataset = []
                # indices = []
                # victim.plm.eval()
                # target_label = poison_dataset["test-poison"][0][1]
                # for idx,(text,label,poison_label) in enumerate(detect_poison_dataset):
                #     input_tensor = victim.tokenizer.encode(text, add_special_tokens=True)
                #     input_tensor = torch.tensor(input_tensor).unsqueeze(0).cuda()
                #     outputs = victim.plm(input_tensor, output_hidden_states=True)
                #     predict_labels = outputs.logits.squeeze().argmax()
                #     if predict_labels==target_label:
                #         protected_poison_dataset.append((text,label,poison_label))
                #         indices.append(idx)
                # detect_poison_dataset = {'test-detect':protected_poison_dataset}
                detect_poison_dataset = {'test-detect': detect_poison_dataset}

                detection_score, preds = defender.eval_detect(model=victim, clean_data=dataset, poison_data=detect_poison_dataset)

                # indices = [idx for idx,pred in zip(indices,preds) if pred==1]
                # preds = [1 if i in indices else 0 for i in range(total_nums)]
        ...
        if detection_score:
            return dict(results[0], **sample_metrics, **detection_score)
        else:
            return dict(results[0], **sample_metrics)

    def get_detect_result(self,victim: Victim, dataset: List, defender: Optional[Defender] = None):

        poison_dataset = self.poison(victim, dataset, "eval")
        detect_poison_dataset = self.poison(victim, dataset, "detect")['test-detect']
        detect_poison_dataset = {'test-detect': detect_poison_dataset}

        detection_score, preds = defender.eval_detect(model=victim, clean_data=dataset,
                                                      poison_data=detect_poison_dataset)

        clean_length = len(poison_dataset["test-clean"])
        preds_clean, preds_poison = preds[:clean_length], preds[clean_length:]

        return preds_clean,preds_poison,detection_score
