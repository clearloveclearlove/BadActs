...
class TomatoProcessor(DataProcessor):
    """
    ...
    def __init__(self):
        super().__init__()
        self.path = "./datasets/SentimentAnalysis/tomato"

    def get_examples(self, data_dir, split):
        examples = []
        if data_dir is None:
            data_dir = self.path
        label_file = open(os.path.join(data_dir, "{}_y.txt".format(split)), 'r')
        labels = [int(x.strip()) for x in label_file.readlines()]
        with open(os.path.join(data_dir, '{}_x.txt'.format(split)),'r') as fin:
...
class YelpProcessor(DataProcessor):
    ...
    def __init__(self):
        super().__init__()
        self.path = "./datasets/SentimentAnalysis/yelp"
