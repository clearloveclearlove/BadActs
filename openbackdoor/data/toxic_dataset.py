...
class TwitterProcessor(DataProcessor):
    ...
    def get_examples(self, data_dir, split):
        ...
        import pandas as pd
        data = pd.read_csv(os.path.join(data_dir, '{}.tsv'.format(split)), sep='\t').values.tolist()
        examples = [(item[0].strip(), int(item[1]), 0) for item in data if isinstance(item[0],str)]
        # sentences = [str(item[0]).strip() for item in data]
        # labels = [int(item[1]) for item in data]
        # examples = [(sentences[i], labels[i], 0) for i in range(len(labels))]
        return examples
...
class ConvidProcessor(DataProcessor):
    """
    `HSOL`_ is a toxic comment classification dataset.
    """
    ...
    def __init__(self):
        super().__init__()
        self.path = "./datasets/Toxic/convid"
    ...
    def get_examples(self, data_dir, split):
        ...
        if data_dir is None:
            data_dir = self.path
        import pandas as pd
        data = pd.read_csv(os.path.join(data_dir, '{}.csv'.format(split))).values.tolist()
        examples = [(item[1].strip(), int(item[2].strip()=='fake'), 0) for item in data if isinstance(item[1], str)]
        return examples
