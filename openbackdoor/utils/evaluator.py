...
class Evaluator:
    ...
    def evaluate_ppl(self, orig_sent_li, poison_sent_li):
        lm = GPT2LM()
        # print(len(orig_sent_li),len(poison_sent_li))
    ...
    def evaluate_use(self, orig_sent_li, poison_sent_li):
        ...
        for i in range(len(orig_sent_li)):
            ...
            poison_sent = poison_sent_li[i]
            if poison_sent != orig_sent:
                all_use.append(use.get_sim(orig_sent, poison_sent))
...
class GPT2LM:
    def __init__(self):
        ...
        self.lm = transformers.GPT2LMHeadModel.from_pretrained("gpt2-large").eval()
    ...
    def __call__(self, sent):
        ...
        if ipt['input_ids'].size(1)==1:
            ipt = self.tokenizer(sent+' <|endoftext|>', return_tensors="pt",
                                 max_length=512, verbose=False)
...
class SentenceEncoder:
    def __init__(self, device='cuda'):
        ...
        '''
        self.model = SentenceTransformer('paraphrase-distilroberta-base-v1', device).eval()
